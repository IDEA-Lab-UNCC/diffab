...
Plots a design energy column for each CDR with its native control alongside.
`--metric` selects which energy:

`dG_separated` (default) reads `summary.csv` from `diffab.tools.eval.run`. Note
that `dG_ref` here is not a single number: `energy.py` recomputes it for every
design, and `InterfaceAnalyzerMover(pack_separated=True)` repacks the separated
state stochastically. In practice the 100 values per CDR land on only 6-16
distinct outcomes and are left-skewed -- the mode sits near the maximum with rare
low excursions -- so a mean and a min-max band misrepresent it. It is drawn as its
own distribution instead.

`dG_bind` and `dG_int` read `binding_per_design.csv` from
`diffab.tools.eval.binding`. Both are measured against one cached reference per
CDR rather than recomputed per design, so their native control is a single value
and the reference spread collapses to a point -- that is the intended
improvement, not a plotting artefact. `dG_bind` repacks both bound and unbound
states; `dG_int` repacks neither and is deterministic. Unlike `dG_separated`,
neither charges the whole structure's packing strain to the binding term.
...
Overlay several runs in one figure:
    ...
    python -m diffab.tools.eval.plot_energy --runs <runA> <runB> --out <runB> \
        --kind violin --metric dG_bind

Runs are never pooled. Each design distribution keeps its own reference,
colour-matched: two runs relaxed from the same native can still disagree on the
reference (`dG_ref` for L_CDR3 differs by 3.5 REU between the 7DK2 runs, ~6x the
within-run spread), and pooling would hide that inside a wider band.
"""
...
# Each metric names the CSV it lives in, its design and native columns, the axis
# label, and the stem for the output filenames. The `dG_separated` stem is `dG`
# so the filenames it has always written stay unchanged.
METRICS = {
    'dG_separated': dict(
        source='summary.csv', gen='dG_gen', ref='dG_ref', stem='dG',
        label='dG_separated (REU)', producer='diffab.tools.eval.run',
    ),
    'dG_bind': dict(
        source='binding_per_design.csv', gen='dG_bind', ref='ref_dG_bind',
        stem='dG_bind', label='dG_bind (REU)',
        producer='diffab.tools.eval.binding',
    ),
    'dG_int': dict(
        source='binding_per_design.csv', gen='dG_int', ref='ref_dG_int',
        stem='dG_int', label='dG_int (REU)',
        producer='diffab.tools.eval.binding',
    ),
}


def filenames(metric):
    """(single-run hist, compare hist, single-run violin, compare violin)."""
    stem = metric['stem']
    return (f'{stem}_distribution.png', f'{stem}_distribution_compare.png',
            f'{stem}_violin.png', f'{stem}_violin_compare.png')


def find_summaries(root, metric):
    """Every source CSV for `metric` under `root`, keyed by the run directory."""
    hits = {}
    pattern = os.path.join(root, '**', metric['source'])
    for path in sorted(glob.glob(pattern, recursive=True)):
        # <run>/evaluation/<source> -> <run>
        hits[os.path.dirname(os.path.dirname(path))] = path
    return hits


def required(metric):
    return {'cdr', metric['gen'], metric['ref']}


def load_run(directory, metric):
    """The metric's source CSV for one run, or None with a reason printed."""
    path = os.path.join(directory, 'evaluation', metric['source'])
    if not os.path.exists(path):
        print(f"  {directory}: no evaluation/{metric['source']} -- "
              f"run {metric['producer']} first")
        return None
    table = pd.read_csv(path)
    missing = required(metric) - set(table.columns)
    if missing:
...
def print_stats(table, metric, label=None):
    if label:
        print(f'\n--- {label} ---')
    gen_col, ref_col = metric['gen'], metric['ref']
    print(f"{'cdr':8}{'n':>5}{gen_col + ' mean':>15}{'median':>9}{'min':>9}{'max':>10}"
          f"{'ref':>9}{'ref sd':>8}{'ref uniq':>10}{'% < ref':>9}")
    for cdr in order_cdrs([table]):
        sub = table[table['cdr'] == cdr]
        gen, ref = sub[gen_col].to_numpy(float), sub[ref_col].to_numpy(float)
        print(f'{cdr:8}{len(gen):5d}{gen.mean():15.2f}{np.median(gen):9.2f}'
...
def make_figure(runs, metric, clip=None, bins=40, title=''):
    ...
    for i, cdr in enumerate(cdrs):
        ...
        for (label, table), colour in zip(runs, COLOURS):
            ...
            per_run.append((label, colour,
                            sub[gen_col].to_numpy(float),
                            sub[ref_col].to_numpy(float)))
        if not per_run:
        ...
        strip.set_xlabel(f"{metric['label']}  —  lower is better", fontsize=8.5)

    fig.suptitle(title, fontsize=12)
    return fig


def make_violin_figure(runs, metric, clip=None, title='', ylim=None):
    ...
    pooled = np.concatenate([t[gen_col].to_numpy(float) for _, t in runs])
    cap = np.percentile(pooled, clip) if clip else None

    handles, hidden, multivalued = [], 0, False
    for j, ((label, table), colour) in enumerate(zip(runs, COLOURS)):
        ...
        for i, cdr in enumerate(cdrs):
            ...
            if sub.empty:
                continue
            gen = sub[gen_col].to_numpy(float)
            ref = sub[ref_col].to_numpy(float)
            ...
            # A cached reference takes one value, so the line already says
            # everything; dots would just sit on top of it.
            if len(uniq) > 1:
                multivalued = True
    ...
    handles.append(plt.Line2D([], [], color='0.25', lw=2.4,
                              label=f"native {ref_col}"
                                    + (' (median)' if multivalued else '')))
    if multivalued:
        handles.append(plt.Line2D([], [], color='0.25', lw=0, marker='o',
                                  markerfacecolor='none',
                                  label=f'{ref_col} values (size = frequency)'))

    ax.set_xticks(range(len(cdrs)))
    ax.set_xticklabels(cdrs)
    ax.set_ylabel(f"{metric['label']}   —  lower is more favourable")
    if ylim:
        ax.set_ylim(*ylim)
...
def main():
    ...
    parser.add_argument('--metric', choices=sorted(METRICS), default='dG_separated',
                        help="which energy to plot: 'dG_separated' (default, from "
                             "summary.csv) or 'dG_bind' / 'dG_int' (from "
                             'binding_per_design.csv)')
    args = parser.parse_args()

    metric = METRICS[args.metric]
    hist_name, hist_compare, violin_name, violin_compare = filenames(metric)
    ...
    if args.runs:
        ...
        for path, label in zip(args.runs, labels):
            table = load_run(path, metric)
            if table is not None:
                runs.append((label, table))
        if not runs:
            return
        for label, table in runs:
            print_stats(table, metric, label)
        title = '  vs  '.join(l for l, _ in runs) + f"   [{args.metric}]"
        if args.kind == 'violin':
            fig = make_violin_figure(runs, metric, clip=args.clip, title=title,
                                     ylim=args.ylim)
            name = violin_compare
        else:
            fig = make_figure(runs, metric, clip=args.clip, bins=args.bins,
                              title=title)
            name = hist_compare
    ...
    summaries = find_summaries(args.root, metric)
    if not summaries:
        print(f"No {metric['source']} found under {args.root}. "
              f"Run {metric['producer']} first.")
        return
    for directory, path in summaries.items():
        ...
        print(f'\n=== {label} ===')
        print_stats(table, metric)
        title = f'{label}   [{args.metric}]'
        if args.kind == 'violin':
            fig = make_violin_figure([(label, table)], metric, clip=args.clip,
                                     title=title, ylim=args.ylim)
            name = violin_name
        else:
            fig = make_figure([(label, table)], metric, clip=args.clip,
                              bins=args.bins, title=title)
            name = hist_name
